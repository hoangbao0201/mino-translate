...
from ..utils.textblock import TextBlock, adjust_blks_size
...
def draw_text(image: np.ndarray, blk_list: List[TextBlock], font_pth: str, colour: str = "#000", init_font_size: int = 40, min_font_size=10, outline: bool = True):
    ...
    blk_list_length = len(blk_list)
    print(f"Độ dài của blk_list: {blk_list_length}")
...
def manual_wrap(image: np.ndarray, blk_list: List[TextBlock]):
    font_family = 'Arial'  # Default font family
    line_spacing = 1.0     # Default line spacing
    outline_width = 1      # Default outline width
    bold = False           # Default bold setting
    italic = False         # Default italic setting
    underline = False      # Default underline setting
    init_font_size = 12    # Default initial font size
    min_font_size = 10     # Default minimum font size
    ...
    for blk in blk_list:
        ...
        translation, font_size = pyside_word_wrap(
            image, (x1, y1), font_family, translation, width, height,
            align='left', spacing=line_spacing, init_font_size=init_font_size, min_font_size=min_font_size
        )
        
        cv2.putText(image, translation, (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, font_size / 20, (0, 255, 0), 2)
